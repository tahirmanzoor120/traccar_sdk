...
class NotificationsApi extends BaseApi {
  ...
  Future<Notification> updateNotification(int id, Notification notification) =>
      guard(() async {
  ...
  Future<List<NotificationType>> getNotificationTypes() => guard(() async {
    final response = await dio.get<List<dynamic>>('/notifications/types');
    return (response.data ?? [])
