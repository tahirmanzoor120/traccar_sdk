...
class SessionApi extends BaseApi {
  ...
  Future<User> getSession({String? token}) => guard(() async {
  ...
  Future<void> logout() => guard(() async => dio.delete<void>('/session'));

  /// Generates a session token with an optional [expiration] date.
  /// Returns the token string.
  Future<String> generateToken({DateTime? expiration}) => guard(() async {
  ...
  Future<void> revokeToken(String token) => guard(
    () async => dio.post<void>(
